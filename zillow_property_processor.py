...
FICO_TO_APR = [
    (620, 7.900), (640, 7.344), (660, 6.906),
    (680, 6.688), (700, 6.507), (760, 6.281)
]
...
def calculate_cash_flow_metrics(purchase_price, rent_estimate, monthly_costs, total_cash_invested, prepaids):
    return {
        'cash_on_cash_return' : MONTHS_IN_YEAR * (rent_estimate - monthly_costs) / total_cash_invested,
        'cash_on_cash_return_no_prepaids' : MONTHS_IN_YEAR * (rent_estimate - monthly_costs) / (total_cash_invested - prepaids),
        'adjusted_cash_on_cash_return' : MONTHS_IN_YEAR * (rent_estimate * (1 - VACANCY_RATE) - monthly_costs - (MONTHLY_MAINTENANCE_RATE * purchase_price)) / total_cash_invested,
        'adjusted_cash_on_cash_return_no_prepaids' : MONTHS_IN_YEAR * (rent_estimate * (1 - VACANCY_RATE) - monthly_costs - (MONTHLY_MAINTENANCE_RATE * purchase_price)) / (total_cash_invested - prepaids)
    }
...
def calculate_real_estate_metrics(base_path='PropertyDetails'):
    results = {}
        ...
        for json_file_path in glob.glob(os.path.join(zip_code_folder, '*_property_details.json')):
            with open(json_file_path, 'r') as json_file:
                ...
                for down_payment_percentage in DOWN_PAYMENT_PERCENTAGES:
                    for fico_score, apr in FICO_TO_APR:
                        ...
                        total_monthly_costs, total_cash_invested, total_prepaid_costs = calculate_monthly_costs(purchase_price, down_payment_percentage, apr, property_info)
                        metrics = {
                            'street_address': property_info.get('streetAddress', 'No Property Address Located'),
                            'zip_code': zip_code,
                            'fico_score': fico_score,
                            'down_payment': f"{down_payment_percentage * 100}%",
                            'purchase_price': purchase_price
                        }
                        metrics.update(calculate_cash_flow_metrics(purchase_price, rent_estimate, total_monthly_costs, total_cash_invested, total_prepaid_costs))
                        metrics.update({
                            'cap_rate' : MONTHS_IN_YEAR * (rent_estimate - total_monthly_costs) / purchase_price,
                            'adjusted_cap_rate' : MONTHS_IN_YEAR * (rent_estimate * (1 - VACANCY_RATE) - total_monthly_costs - (MONTHLY_MAINTENANCE_RATE * purchase_price)) / purchase_price,
                            'gross_rent_multiplier' : purchase_price / (MONTHS_IN_YEAR * rent_estimate) if rent_estimate != 0 else 'inf',
                            'break_even_ratio' : (total_monthly_costs * 12 + annual_debt_service) / (MONTHS_IN_YEAR * rent_estimate) if rent_estimate != 0 else 'inf'
                        })
                        results.append(metrics)

    # Sort the list of dictionaries for the current zip code by 'cash_on_cash_return', in descending order
    results.sort(key=lambda x: x['adjusted_cash_on_cash_return'], reverse=True)

    # Save the results to a JSON file
    save_json(results, 'processed_property_metric_results.json')
