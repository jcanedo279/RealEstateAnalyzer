...
def properties_response_from_properties_df(properties_df, number_deals=1):
    ...
    properties_df = properties_df[:number_deals]
...
def search():
    ...
    number_deals = int(request_data.get('num_deals'))
    properties_df = properties_df_from_search_request_data(request_data)

    response_data = properties_response_from_properties_df(properties_df, number_deals=number_deals)
...
def direct_search():
    ...
    zpid = int(data.get('property_id', 0))
    
    if zpid in BACKEND_PROPERTIES_DF.index:
        properties_df = BACKEND_PROPERTIES_DF.loc[[zpid]].copy()
