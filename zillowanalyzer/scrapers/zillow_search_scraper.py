...
def scrape_listings_in_zip_code(zip_code_index, zip_code):
    zip_code_path = f"{HOME_DATA_PATH}/{zip_code}"
    metadata_path = f"{zip_code_path}/metadata.json"

    with get_selenium_driver(f"https://www.zillow.com/homes/{zip_code}_rb/") as driver:
        cookie_string = extract_cookies_from_driver(driver, scrape_config['2s_delay'])
    ...
    for page in range(1,max_pages_requested_per_zip+1):
        ...
        save_json(listings, f'{zip_code_path}/listings_page_{page}.json')
